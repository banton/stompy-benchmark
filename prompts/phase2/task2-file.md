# Task 2 — Extract context_explore & context_dashboard to Services (File Memory Condition)
...
1. **Read `MEMORY.md`** in the project root. It contains comprehensive architecture notes including:
   - Service extraction patterns and conventions
   - Database access patterns (postgres_adapter, get_db helpers)
   - MCP tool reference (including context_explore and context_dashboard)
   - Testing conventions and fixture patterns

2. **Read `TASKS.md`** in the project root. It contains the project ticket board with related tasks.

3. **Key reference**: Look at how `context_service.py` and `context_search_service.py` were previously extracted — they are the template for this extraction. MEMORY.md should describe this pattern.

4. Only explore files directly if MEMORY.md doesn't cover what you need.
