# Task 1 — project_delete Preview with Data Counts (No Memory Condition)
...
1. Look at the project structure — list files in `src/`, `src/services/`, `src/middleware/`, `src/api/routes/`, and `tests/`
2. Read `CLAUDE.md` for project-level guidance
3. Read `stompy_server.py` to find the `project_delete` tool implementation
4. Read `src/services/project_service.py` to understand service patterns
5. Read `postgres_adapter.py` to understand database access patterns
6. Read `src/migrations/definitions.py` to understand the database schema (table names, columns)
7. Look at existing test files to understand testing patterns and conventions
