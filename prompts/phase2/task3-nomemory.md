# Task 3 — Per-Account Rate Limiting with Age-Based Thresholds (No Memory Condition)
...
1. Look at the project structure — list files in `src/`, `src/services/`, `src/middleware/`, `src/api/routes/`, `src/config/`, and `tests/`
2. Read `CLAUDE.md` for project-level guidance
3. Read `src/middleware/rate_limit.py` to understand current rate limiting
4. Read `src/middleware/auth.py` to understand authentication and user identification
5. Read `server_hosted.py` to understand how middleware and routes are registered
6. Read `src/api/routes/` files to understand route registration patterns
7. Read `src/api/dependencies.py` or `src/api/router.py` to understand dependency injection
8. Read `src/_config.py` or `src/config/` to understand configuration patterns
9. Look at Redis usage — search for "redis" across the codebase to understand connection patterns
10. Read existing test files to understand testing patterns (especially rate limit or middleware tests)
11. Read `src/models/user.py` to understand user/account data structures
